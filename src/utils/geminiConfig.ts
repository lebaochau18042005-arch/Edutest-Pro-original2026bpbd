...
export const GEMINI_ERROR_MESSAGES = {
  MISSING_KEY: "Vui lòng cấu hình API Key trước khi sử dụng tính năng này.",
  INVALID_KEY: "API Key không hợp lệ hoặc đã hết hạn. Vui lòng kiểm tra lại trong Settings.",
  PERMISSION_DENIED: "API key không có quyền truy cập Gemini API. Hãy tạo auth key mới trong Google AI Studio.",
  QUOTA_EXCEEDED: "Đã hết quota hoặc vượt giới hạn tốc độ API. Vui lòng đợi một lúc rồi thử lại.",
  MODEL_OVERLOADED: "Model Google đang quá tải hoặc tạm thời không khả dụng. App đang thử model dự phòng.",
} as const;
