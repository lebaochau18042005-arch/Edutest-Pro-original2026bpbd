...
export function submissionToRowObject(s: StudentSubmission, idx: number) {
  ...
  return {
    ...
    "Mã Đề": s.examCode,
    "Điểm Tổng (10.0)": typeof s.score === "number" ? Number(s.score.toFixed(2)) : 0,
    "Điểm Phần I": typeof s.part1Score === "number" ? Number(s.part1Score.toFixed(2)) : "-",
    "Điểm Phần II": typeof s.part2Score === "number" ? Number(s.part2Score.toFixed(2)) : "-",
    "Điểm Phần III": typeof s.part3Score === "number" ? Number(s.part3Score.toFixed(2)) : "-",
    "Số Câu Đúng": s.correctCount,
    "Số Câu Sai": s.wrongCount,
    "Chưa Làm": s.unansweredCount,
    "Tổng Số Câu": s.totalQuestions,
    "Thời Gian Làm (phút)": Number(((s.durationTakenSeconds || 0) / 60).toFixed(1)),
...
export function generateGoogleSheetsTSVByClass(
  ...
  filtered.forEach((s, idx) => {
    ...
    const row = [
      ...
      typeof s.part1Score === "number" ? s.part1Score.toFixed(2) : "-",
      typeof s.part2Score === "number" ? s.part2Score.toFixed(2) : "-",
      typeof s.part3Score === "number" ? s.part3Score.toFixed(2) : "-",
      s.correctCount,
      s.wrongCount,
      s.unansweredCount,
      s.totalQuestions,
      ((s.durationTakenSeconds || 0) / 60).toFixed(1),
      s.tabSwitchCount,
      totalViolations,
      isCheating
...
export async function syncSubmissionToGoogleSheetsWebhook(
  ...
  try {
    const payload = {
      ...
      durationTakenMinutes: Number(((submission.durationTakenSeconds || 0) / 60).toFixed(1)),
